...
int main() {
	...
	for(unsigned int i = 0 ; i < pbs.numbersToSort.size() ; ++i){
		pleb.numbersToSort.push_back(pbs.numbersToSort.at(i));
	}
	pleb.sortNormal();

	
	for(unsigned int i = 0 ; i < totalNumbers ; ++i){
		if(pleb.numbersToSort.at(i) != pbs.numbersToSort.at(i)){
			std::cout << pleb.numbersToSort.at(i) << "!=" << pbs.numbersToSort.at(i) << "\n";
		}
	}
